...
describe("stick-pack content and pacing refinements", () => {
  ...
  it("uses clear Compounding Effect milestones without the rejected timing claims", () => {
    ...
    const stages = source.slice(
      source.indexOf("const COMPOUNDING_STAGES"),
      source.indexOf("/* --- Comparison rows --- */"),
    );
  ...
  it("shows bag-based offers in ascending order with dollar savings and three bags preselected", () => {
    ...
    expect(source).not.toContain('useState("1mo")');
    expect(source).not.toContain('useState("2mo")');
    expect(source).toContain('useState("3mo")');
